...
class RuntimeTracker(object):
    ...
    def state_dict(self):
        return {
            ...
            "stat": {
                metric_name: dict(vars(meter))
                for metric_name, meter in self.stat.items()
            },
        }

    def load_state_dict(self, state):
        if list(state["metrics_to_track"]) != list(self.metrics_to_track):
            raise RuntimeError(
                "Metric checkpoint mismatch: "
                f"checkpoint={state['metrics_to_track']}, current={self.metrics_to_track}."
            )
        for metric_name, meter_state in state["stat"].items():
            if metric_name not in self.stat:
                raise RuntimeError(
                    f"Metric {metric_name} is missing from the current tracker."
                )
            for key, value in meter_state.items():
                setattr(self.stat[metric_name], key, value)
