...
class BaseAdaptation(object):
    ...
    def get_checkpoint_state(self) -> Dict[str, Any]:
        """Return the online state required to continue a CTTA stream.

        Top-level online modules and optimizers are discovered automatically.
        Base/source copies are reconstructed during normal initialization and are
        omitted to keep recovery checkpoints reasonably small. Methods with
        additional non-module online state can override the extra-state hooks.
        """
        module_states = {}
        module_training = {}
        optimizer_states = {}
        for name, value in vars(self).items():
            if isinstance(value, nn.Module) and name != "_base_model":
                module_states[name] = value.state_dict()
                module_training[name] = value.training
            elif (
                isinstance(value, torch.optim.Optimizer)
                and name not in ["_base_optimizer", "ewc_optimizer"]
            ):
                optimizer_states[name] = value.state_dict()

        standard_extra_state = {}
        for name in [
            "oracle_adaptation_steps",
            "optimal_adaptation_steps",
            "nbstep_ratio",
            "fishers",
        ]:
            if hasattr(self, name):
                standard_extra_state[name] = getattr(self, name)

        return {
            "modules": module_states,
            "module_training": module_training,
            "optimizers": optimizer_states,
            "standard_extra_state": standard_extra_state,
            "method_extra_state": self._get_checkpoint_extra_state(),
        }
    ...
    def load_checkpoint_state(self, state: Dict[str, Any]) -> None:
        for name, module_state in state.get("modules", {}).items():
            ...
            if module is None:
                raise RuntimeError(
                    f"Recovery checkpoint contains missing adaptation module {name}."
                )
            module.load_state_dict(module_state)
            module.train(state.get("module_training", {}).get(name, module.training))

        for name, optimizer_state in state.get("optimizers", {}).items():
            optimizer = getattr(self, name, None)
            if optimizer is None:
                raise RuntimeError(
                    f"Recovery checkpoint contains missing optimizer {name}."
                )
            optimizer.load_state_dict(optimizer_state)

        for name, value in state.get("standard_extra_state", {}).items():
            setattr(self, name, value)
        self._load_checkpoint_extra_state(state.get("method_extra_state", {}))
    ...
    def _load_checkpoint_extra_state(self, state: Dict[str, Any]) -> None:
        if state:
            raise RuntimeError(
                f"{self.__class__.__name__} does not support checkpoint extra state: "
                f"{list(state.keys())}"
            )
